...
## flatten field
```bash
curl -XPUT 'http://localhost:9200/_ingest/pipeline/phone_numbers' -H 'Content-Type: application/json' --data-binary @./transform_test/master_plan/number_pipeline.json
```
...
## preview
```bash
curl -XPOST 'http://localhost:9200/_data_frame/transforms/_preview?pretty' -H 'Content-Type: application/json' --data-binary @./transform_test/master_plan/transform.json
```
    ...
    "mappings" : {
      "_meta" : {
        "_transform" : {
          ...
          "creation_date_in_millis" : 1730519418823
        },
...
## create
```bash
curl -XPUT 'http://localhost:9200/_transform/test-transform1-8?pretty' -H 'Content-Type: application/json' --data-binary @./transform_test/master_plan/transform.json
```

## start
```bash
curl -XPOST 'http://localhost:9200/_transform/test-transform1-8/_start'
```

## transform status
```bash
curl -XGET 'http://localhost:9200/_transform/test-transform1-8/_stats?pretty'
```
...
## search
```bash
curl -XGET 'http://localhost:9200/customer_phone/_search?pretty' -H 'Content-Type: application/json' --data-binary @./transform_test/master_plan/search_from_transform_index.json
```
